...
import {
  DevicesOther as DevicesIcon, Security as SecurityIcon,
  CheckCircle as CheckIcon, Warning as WarningIcon,
  TrendingUp as TrendUpIcon, TrendingDown as TrendDownIcon,
} from '@mui/icons-material';

const iconMap = { devices: DevicesIcon, security: SecurityIcon, check: CheckIcon, warning: WarningIcon };
...
const StatCard = ({ title, value, subtitle, icon, trend, trendUp, color }) => {
  ...
  useEffect(() => {
    ...
    if (isNaN(numValue)) { setDisplayValue(value); return; }
    let start = 0;
  ...
  return (
    <Card sx={{ height: '100%', position: 'relative', overflow: 'hidden', '&::before': {
      ...
      <CardContent sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
          <Box sx={{ p: 1.5, borderRadius: 3, bgcolor: `${color}.main`, opacity: 0.1 }}>
            <Icon sx={{ color: `${color}.main`, fontSize: 28 }} />
          </Box>
          <Chip
            icon={trendUp ? <TrendUpIcon /> : <TrendDownIcon />}
            label={trend} size="small"
            sx={{ bgcolor: trendUp ? 'success.main' : 'error.main', color: 'white', fontWeight: 600 }}
          />
        </Box>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ opacity: 0.7 }}>{subtitle}</Typography>
      </CardContent>
    </Card>
  );
};
